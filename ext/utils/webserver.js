...
for (let entryName in config.entry) {
  if (excludeEntriesToHotReload.indexOf(entryName) === -1) {
    ...
    config.entry[entryName] = ['webpack/hot/dev-server.js', `webpack-dev-server/client/index.js?hot=true&hostname=localhost&port=${env.PORT}`].concat(config.entry[entryName]);
  }
}
